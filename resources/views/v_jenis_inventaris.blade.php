          ...
          <div class="row">
            <div class="col-md-6">
              <div class="control-group">
                <label class="control-label">Kode Barang Inventaris</label>
                <div class="controls">
                  <input class="form-control" type="text" id="kode_barang" placeholder="Type something here..." class="span15">
                </div>
              </div>
            </div>
            <div class="col-md-6">
              <div class="control-group">
                ...
                <div class="controls">
                  <input class="form-control" type="text" id="jenis_barang" placeholder="Type something here..." class="span15">
                </div>
              </div>
            </div>
          </div>
...
<script>
  ...
  $('.modal-footer').on('click', '.add', function() {
    $.ajax({
      type: 'POST',
      url: '{{ url("store_jenis_inventaris") }}',
      data: {
        '_token': "{{ csrf_token() }}",
  ...
  $('.modal-footer').on('click', '.update', function() {
    $.ajax({
      type: 'PUT',
      url: 'update_jenis_inventaris/' + id,
      data: {
        '_token': "{{ csrf_token() }}",
        'kode_barang': $('#kode_barang').val(),
        'jenis_barang': $('#jenis_barang').val(),
  ...
  $(document).on('click', '#delete', function(e) {
    ...
    if (confirm('Yakin akan menghapus data ini?')) {
      ...
      $.ajax({
        type: 'DELETE',
        url: 'delete_jenis_inventaris/' + $(this).data('id'),
        data: {
          '_token': "{{ csrf_token() }}",
        },
        success: function(data) {
